...
export function DriverForm() {
  ...
  const [f, setF] = useState<Record<string, string>>({ name: "", phone: "", email: "", city: "", position: "Box truck driver (non-CDL)", license_class: "", endorsements: "", experience: "", equipment: "", availability: "", mvr: "", notes: "", consent: "" });
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); submit(f); }} className="card relative overflow-hidden p-6 sm:p-8" id="apply">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div><label className="label" htmlFor="d-po">Applying as</label><select id="d-po" className="field" value={f.position} onChange={(e) => set("position", e.target.value)}><option>Box truck driver (non-CDL)</option><option>Cargo van driver (non-CDL)</option><option>Flatbed / dry van driver (CDL)</option><option>Owner-operator</option><option>Dispatch / office</option></select></div>
        ...
        <div><label className="label" htmlFor="d-eq">Equipment experience</label><input id="d-eq" className="field" placeholder="e.g. box truck, flatbed, dry van, sprinter" value={f.equipment} onChange={(e) => set("equipment", e.target.value)} /></div>
