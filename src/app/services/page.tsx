...
export default function ServicesPage() {
  return (
    <>
      ...
      <PageHero eyebrow="Services" title="Trucking services built around the load" sub="Six services, three unit types, one dispatcher. Every option below runs on our own trucks with our own CDL drivers." crumbs={[{ name: "Services", path: "/services/" }]}>
        ...
        <Container>
          ...
          <div className="reveal mt-12 overflow-x-auto rounded-2xl border border-line">
            <table className="w-full min-w-[720px] text-left text-[14px]">
              <thead className="bg-navy text-white"><tr>{["Service", "Equipment", "Capacity", "Turnaround", "Best for"].map((h) => <th key={h} className="px-4 py-3 text-[12px] font-semibold uppercase tracking-wider">{h}</th>)}</tr></thead>
              <tbody>
                {services.map((s, i) => (
                  <tr key={s.slug} className={i % 2 ? "bg-cloud" : "bg-white"}>
                    ...
                    <td className="px-4 py-3 text-slate">{s.specs.find((x) => /equip|deck|box|cargo/i.test(x.label))?.value ?? s.specs[0].value}</td>
                    ...
                    <td className="px-4 py-3 text-slate">{s.fit[0]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
